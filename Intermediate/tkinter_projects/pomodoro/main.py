...
def reset_timer():
    ...
    # timer_label reads "Timer"
    timer_label.config(text="Timer", fg=GREEN)

    # Erase all checkmarks
    global checks
    checks = ""
    tracker.config(text=checks)
...
def start_timer():
    ...
    if reps % 8 == 0:
        ...
        count_down(short_break_sec) ## enter 3 sec for testing
        timer_label.config(text="Break", fg=PINK)
    else:
        count_down(work_sec) ## enter 5 sec for testing
...
# tracker label
tracker = Label(fg=GREEN, bg=YELLOW)
